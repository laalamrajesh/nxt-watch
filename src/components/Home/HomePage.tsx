...
const Home = observer(() => {
  ...
  if (videoStore.errorMsg) {
    return <ErrorComponent message={videoStore.errorMsg} retry={videoStore.fetchVideos} />;
  }
  ...
  return (
    <HomeContainer>
      ...
      <HomepageMainDiv>
      <Sidebar onNavigate={(path: string) => history.push(path)} page="home" />
      <MainContent>
        <SearchBar onSearch={videoStore.fetchVideos} theme="light" />

        <VideoGroup>

        {videoStore.videos.length > 0 ? (
          videoStore.videos.map((video: any) => (
            <VideoCard key={video.id} video={video} page="home" onClick={() => history.push(`/videos/${video.id}`)} />
          ))
        ) : (
          <NoResults />
        )}
